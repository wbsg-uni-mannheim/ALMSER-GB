...
class MatchingTask(object):
    ...
    def __init__(self, source_dt, target_dt, gs, gs_feature_vector, common_attributes_names, feature_vector_train=None,feature_vector_test=None,feature_vector_validation=None):
    ...
    def getProfilingFeatures(self):
        ...
        for feat, weight in zip(features_in_order, feature_weights):
            if weight>0: 
                ...
                corr_attr = get_cor_attribute(self.common_attributes_names,feat)
                if (corr_attr not in matching_relevant_attributes): #so that we keep the order
        ...
        self.dict_profiling_features['matching_relevant_attributes'] = matching_relevant_attributes
        self.dict_profiling_features['matching_relevant_attributes_count'] = len(matching_relevant_attributes)
    ...
    def getNestedXValidationResults(self, model):
        print("Get Nested X-Validation Results")
